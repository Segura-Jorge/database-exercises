...
SELECT 
    *
FROM
    sakila_payment;CREATE TABLE avg_dept_salaries SELECT employees.d.dept_name AS 'dept_name',
    AVG(employees.s.salary) AS 'average_salary' FROM
    employees.employees AS e
        JOIN
    employees.dept_emp AS de ON employees.e.emp_no = employees.de.emp_no
        JOIN
    employees.departments AS d ON employees.d.dept_no = employees.de.dept_no
        JOIN
    employees.salaries AS s ON employees.s.emp_no = employees.e.emp_no
WHERE
    employees.de.to_date > CURDATE()
        AND employees.s.to_date > CURDATE()
GROUP BY employees.d.dept_name;

CREATE TABLE overall_average AS SELECT AVG(employees.s.salary) AS overall_average FROM
    employees.salaries AS s
WHERE
    employees.s.to_date > NOW();

CREATE TABLE overall_std AS SELECT STD(employees.s.salary) AS overall_std FROM
    employees.salaries AS s
WHERE
    employees.s.to_date > NOW();
...
CREATE TABLE avg_dept_salaries_2 SELECT employees.d.dept_name AS 'dept_name',
    AVG(employees.s.salary) AS 'average_salary' FROM
    employees.employees AS e
        JOIN
    employees.dept_emp AS de ON employees.e.emp_no = employees.de.emp_no
